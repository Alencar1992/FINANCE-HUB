...
function FinancialIntelligence({owner,tx,notify,refresh,ask}){
  const[tab,setTab]=useState("visao"),[goals,setGoals]=useState([]),[goalOpen,setGoalOpen]=useState(false),[simulation,setSimulation]=useState(null),[aiLoading,setAiLoading]=useState(false),[aiResult,setAiResult]=useState(null),[aiQuestion,setAiQuestion]=useState("");
  ...
  async function runAI(mode,question=""){setAiLoading(true);setAiResult(null);const{data,error}=await supabase.functions.invoke("financial-ai",{body:{mode,question}});setAiLoading(false);if(error||data?.erro)return notify(data?.erro||"Não foi possível consultar a Inteligência Financeira.");setAiResult({mode,...data.resultado});if(mode!=="assistant")setTab("assistente")}
  ...
  {tab==="assistente"&&<section className="assistant-live"><div className="assistant-title"><BrainCircuit/><div><small>ASSISTENTE PROTEGIDO</small><h3>Converse com suas finanças</h3><p>As respostas usam somente resumos autorizados e são educativas.</p></div></div><form onSubmit={e=>{e.preventDefault();runAI("assistant",aiQuestion)}}><textarea value={aiQuestion} onChange={e=>setAiQuestion(e.target.value)} maxLength="600" required placeholder="Ex.: Em quais categorias gastei mais neste mês?"/><div><small>{aiQuestion.length}/600</small><button className="primary" disabled={aiLoading||!aiQuestion.trim()}>{aiLoading?<RefreshCw className="spin"/>:<Send/>}{aiLoading?"Analisando...":"Perguntar ao Gemini"}</button></div></form>{aiResult&&<article className="ai-answer"><span><Sparkles/><small>{aiResult.mode==="savings"?"SUGESTÕES DE ECONOMIA":aiResult.mode==="monthly_comparison"?"COMPARAÇÃO MENSAL":"RESPOSTA DO ASSISTENTE"}</small></span><h3>{aiResult.titulo}</h3><p>{aiResult.resumo}</p>{aiResult.recomendacoes?.length>0&&<div><strong>Recomendações</strong><ul>{aiResult.recomendacoes.map((item,index)=><li key={index}>{item}</li>)}</ul></div>}{aiResult.alertas?.length>0&&<div className="ai-alerts"><strong>Pontos de atenção</strong><ul>{aiResult.alertas.map((item,index)=><li key={index}>{item}</li>)}</ul></div>}<small className="ai-disclaimer">{aiResult.aviso}</small></article>}</section>}
  </div>
}
function StreamingsModule({owner,notify}){
  ...
  const reference=monthStart(),months=Array.from({length:4},(_,i)=>{const d=new Date();d.setMonth(d.getMonth()-(3-i));return monthStart(d)});
