...
function AccessDetailsModal({detail,profiles,devices,events,onClose}){
  const profile=profiles.find(item=>item.id===detail.profileId),allDevices=devices.filter(item=>item.profile_id===detail.profileId),profileEvents=events.filter(item=>item.profile_id===detail.profileId),items=detail.type==="active"?allDevices.filter(item=>item.status!=="blocked"):allDevices;
  const eventLabels={residence_update_request:"Atualização de residência",temporary_access_code:"Código temporário",unknown_access:"Acesso desconhecido",profile_blocked:"Perfil bloqueado"};
  const title=detail.type==="alerts"?"Alertas registrados":detail.type==="active"?"Aparelhos ativos":"Aparelhos vinculados";
  return <div className="plan-modal-bg" role="dialog" aria-modal="true"><section className="plan-modal access-detail-modal"><div><span><small>{profile?.subscriptions?.name||"Netflix"}</small><h3>{title}</h3><p>{profile?.profile_name} · {profile?.participant_name}</p></span><button type="button" onClick={onClose} aria-label="Fechar detalhes"><X/></button></div>{detail.type==="alerts"?<div className="access-alert-history">{profileEvents.map(event=><article key={event.id}><span><Mail/><strong>{eventLabels[event.event_type]||event.event_type}</strong><time>{new Date(event.received_at).toLocaleString("pt-BR")}</time></span>{event.email_subject&&<p><b>Assunto:</b> {event.email_subject}</p>}{event.sender_email&&<p><b>Remetente:</b> {event.sender_email}</p>}{event.notes&&<p><b>Observações:</b> {event.notes}</p>}</article>)}{!profileEvents.length&&<p className="access-detail-empty">Nenhum alerta registrado.</p>}</div>:<div className="access-detail-devices">{items.map(device=><article key={device.id}><MonitorSmartphone/><span><strong>{device.nickname}</strong><small>{device.device_type} {device.device_identifier?`· ${device.device_identifier}`:""}</small>{device.last_seen_at&&<time>Último acesso: {new Date(device.last_seen_at).toLocaleString("pt-BR")}</time>}</span><i className={device.status}>{device.status==="blocked"?"Bloqueado":"Ativo"}</i></article>)}{!items.length&&<p className="access-detail-empty">Nenhum aparelho nesta situação.</p>}</div>}</section></div>
}

function StreamingAccessBeta({owner,subscriptions,notify}){
  const[profiles,setProfiles]=useState([]),[devices,setDevices]=useState([]),[events,setEvents]=useState([]),[editing,setEditing]=useState(null),[formOpen,setFormOpen]=useState(false),[deviceOpen,setDeviceOpen]=useState(null),[eventOpen,setEventOpen]=useState(null),[detailOpen,setDetailOpen]=useState(null),[blocking,setBlocking]=useState(null);
  ...
  useEffect(()=>{load()},[owner.id]);
  function openProfile(profile=null,source=null){setEditing(profile||{subscription_id:source?.subscription.id||netflixSubscriptions[0]?.id||"",participant_name:source?.person.name||"",profile_name:source?.person.name||"",profile_email:"",notes:"",pin_enabled:false,status:"active"});setFormOpen(true);setTimeout(()=>document.querySelector(".access-form")?.scrollIntoView({behavior:"smooth",block:"start"}),80)}
  ...
  return <section className="access-beta"><div className="access-beta-head"><span><LockKeyhole/></span><div><small>BETA · CONTROLE ASSISTIDO</small><h2>Perfis e aparelhos Netflix</h2><p>Um perfil pode ter vários aparelhos. O bloqueio em lote marca todos e abre a confirmação oficial da Netflix.</p></div><button className="primary" onClick={()=>openProfile()}><Plus/>Novo perfil</button></div>{pendingParticipants.length>0&&<div className="access-import"><div><strong>Dados existentes encontrados</strong><small>Configure os participantes já cadastrados na Netflix.</small></div>{pendingParticipants.map(source=><button key={profileKey(source.subscription.id,source.person.name)} onClick={()=>openProfile(null,source)}><Plus/>{source.person.name}</button>)}</div>}{formOpen&&<form className="inline-form access-form" onSubmit={saveProfile}><div className="modal-head wide"><div><h3>{editing?.id?"Editar perfil":"Configurar perfil"}</h3><p>Todos os campos poderão ser alterados depois.</p></div><button type="button" onClick={closeProfile}><X/></button></div><label>Assinatura<select name="subscription_id" defaultValue={editing?.subscription_id} required>{netflixSubscriptions.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Pessoa responsável<input name="participant_name" defaultValue={editing?.participant_name||""} required/></label><label>Nome do perfil Netflix<input name="profile_name" defaultValue={editing?.profile_name||""} required/></label><label>E-mail do perfil<input name="profile_email" type="email" defaultValue={editing?.profile_email||""} placeholder="Opcional"/></label><label>Status<select name="status" defaultValue={editing?.status||"active"}><option value="active">Ativo</option><option value="review">Em análise</option><option value="blocked">Bloqueado</option></select></label><label className="setting-row"><span><strong>PIN ativado?</strong><small>Controle informativo</small></span><input name="pin_enabled" type="checkbox" defaultChecked={editing?.pin_enabled}/></label><label className="wide">Observações<textarea name="notes" defaultValue={editing?.notes||""}/></label><div className="form-actions wide"><button type="button" onClick={closeProfile}>Cancelar</button><button className="primary">Salvar perfil</button></div></form>}<div className="access-profile-grid">{profiles.map(profile=>{const linked=devices.filter(item=>item.profile_id===profile.id),profileEvents=events.filter(item=>item.profile_id===profile.id),activeDevices=linked.filter(item=>item.status!=="blocked");return <article key={profile.id} className={`access-profile ${profile.status}`}><header><span>{profile.profile_name?.[0]||"N"}</span><div><small>{profile.subscriptions?.name||"Netflix"}</small><h3>{profile.profile_name}</h3><p>{profile.participant_name}{profile.profile_email?` · ${profile.profile_email}`:""}</p></div><i>{statusLabel[profile.status]}</i></header><div className="access-stats"><button onClick={()=>setDetailOpen({profileId:profile.id,type:"devices"})}><MonitorSmartphone/><b>{linked.length}</b><small>Aparelhos</small><ChevronRight/></button><button onClick={()=>setDetailOpen({profileId:profile.id,type:"active"})}><ShieldCheck/><b>{activeDevices.length}</b><small>Ativos</small><ChevronRight/></button><button onClick={()=>setDetailOpen({profileId:profile.id,type:"alerts"})}><Mail/><b>{profileEvents.length}</b><small>Alertas</small><ChevronRight/></button></div><div className="access-device-list">{linked.map(device=><div key={device.id}><MonitorSmartphone/><span><b>{device.nickname}</b><small>{({tv:"Smart TV",mobile:"Celular",computer:"Computador",tablet:"Tablet",other:"Outro"})[device.device_type]}{device.device_identifier?` · ${device.device_identifier}`:""}</small></span><i className={device.status}>{device.status==="blocked"?"Bloqueado":"Ativo"}</i><button onClick={()=>setDeviceOpen({profileId:profile.id,device})} aria-label="Editar aparelho"><Pencil/></button><button onClick={()=>deleteDevice(device)} aria-label="Excluir aparelho"><Trash2/></button></div>)}{!linked.length&&<p>Nenhum aparelho cadastrado.</p>}</div><footer><button onClick={()=>setDeviceOpen({profileId:profile.id,device:null})}><Plus/>Aparelho</button><button onClick={()=>setEventOpen(profile.id)}><Mail/>Registrar alerta</button><button onClick={()=>openProfile(profile)}><Pencil/>Editar</button><a className="netflix-manage-link" aria-label="Abrir gerenciamento de acessos da Netflix" href="https://www.netflix.com/manageaccountaccess" target="_blank" rel="noreferrer"><ExternalLink/>Gerenciar Netflix</a>{profile.status==="blocked"?<button className="restore" onClick={()=>restoreProfile(profile)}><RefreshCw/>Reativar</button>:<button className="block" disabled={blocking===profile.id} onClick={()=>blockProfile(profile)}><LockKeyhole/>{blocking===profile.id?"Bloqueando...":"Bloquear perfil"}</button>}</footer>{profile.last_request_at&&<div className="access-last-alert"><AlertTriangle/>Última solicitação: {new Date(profile.last_request_at).toLocaleString("pt-BR")}</div>}</article>})}</div>{deviceOpen&&<div className="plan-modal-bg"><form className="plan-modal" onSubmit={saveDevice}><div><h3>{deviceOpen.device?"Editar aparelho":"Novo aparelho"}</h3><button type="button" onClick={()=>setDeviceOpen(null)}><X/></button></div><label>Nome do aparelho<input name="nickname" defaultValue={deviceOpen.device?.nickname||""} placeholder="TV da sala" required/></label><label>Tipo<select name="device_type" defaultValue={deviceOpen.device?.device_type||"tv"}><option value="tv">Smart TV</option><option value="mobile">Celular</option><option value="computer">Computador</option><option value="tablet">Tablet</option><option value="other">Outro</option></select></label><label>Identificação<input name="device_identifier" defaultValue={deviceOpen.device?.device_identifier||""} placeholder="Ex.: Samsung sala"/></label><label>Último acesso<input name="last_seen_at" type="datetime-local" defaultValue={deviceOpen.device?.last_seen_at?.slice(0,16)||""}/></label><label>Observações<textarea name="notes" defaultValue={deviceOpen.device?.notes||""}/></label><button className="primary">Salvar aparelho</button></form></div>}{eventOpen&&<div className="plan-modal-bg"><form className="plan-modal" onSubmit={saveEvent}><div><h3>Registrar alerta recebido</h3><button type="button" onClick={()=>setEventOpen(null)}><X/></button></div><label>Tipo<select name="event_type"><option value="residence_update_request">Atualização de residência</option><option value="temporary_access_code">Código de acesso temporário</option><option value="unknown_access">Acesso desconhecido</option></select></label><label>Assunto do e-mail<input name="email_subject" placeholder="Cole o assunto recebido"/></label><label>Remetente<input name="sender_email" type="email" placeholder="info@account.netflix.com"/></label><label>Data e hora<input name="received_at" type="datetime-local" defaultValue={new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16)}/></label><label>Observações<textarea name="notes"/></label><button className="primary">Registrar e marcar para análise</button></form></div>}{detailOpen&&<AccessDetailsModal detail={detailOpen} profiles={profiles} devices={devices} events={events} onClose={()=>setDetailOpen(null)}/>}</section>
}

const exportGroups=[
  {id:"movements",label:"Movimentações e obrigações",tables:[["transactions","Movimentações","transaction_date"],["obligations","Obrigações","next_due_date"]]},
  {id:"cards",label:"Cartões e compras",tables:[["cards","Cartões","created_at"],["card_purchases","Compras dos cartões","first_due_date"]]},
  {id:"subscriptions",label:"Assinaturas e cobranças",tables:[["subscriptions","Assinaturas","created_at"],["subscription_charges","Cobranças de assinaturas","due_date"]]},
  {id:"investments",label:"Investimentos e metas",tables:[["investments","Investimentos","invested_at"],["investment_snapshots","Posições dos investimentos","reference_month"],["financial_goals","Metas financeiras","target_date"]]},
  {id:"salary",label:"Salário",tables:[["salary_settings","Configurações de salário","updated_at"],["salary_events","Eventos de salário","reference_month"]]},
  {id:"functions",label:"Funções personalizadas",tables:[["custom_modules","Funções personalizadas","created_at"],["custom_module_entries","Registros personalizados","created_at"]]},
  {id:"streaming",label:"Streamings, perfis e aparelhos",tables:[["streaming_access_profiles","Perfis de streaming","created_at"],["streaming_access_devices","Aparelhos de streaming","created_at"],["streaming_access_events","Alertas de streaming","received_at"]]},
];
function DataExportPanel({owner,notify}){
  const[group,setGroup]=useState("all"),[start,setStart]=useState(""),[end,setEnd]=useState(""),[busy,setBusy]=useState("");
  const selected=group==="all"?exportGroups:exportGroups.filter(item=>item.id===group);
  async function collect(){const specs=selected.flatMap(item=>item.tables),endExclusive=end?new Date(`${end}T12:00:00`):null;if(endExclusive)endExclusive.setDate(endExclusive.getDate()+1);return Promise.all(specs.map(async([table,label,dateField])=>{let query=supabase.from(table).select("*").eq("owner_id",owner.id).order(dateField,{ascending:true});if(start)query=query.gte(dateField,start);if(endExclusive)query=query.lt(dateField,endExclusive.toISOString().slice(0,10));const{data,error}=await query;if(error)throw new Error(label);return{table,label,rows:data||[]}}))}
  async function run(format){
    if(start&&end&&start>end)return notify("A data inicial não pode ser posterior à data final.");setBusy(format);
    try{const datasets=await collect(),stamp=new Date().toISOString().slice(0,10),nonEmpty=datasets.filter(item=>item.rows.length);if(!nonEmpty.length)return notify("Nenhum dado foi encontrado para os filtros escolhidos.");
      if(format==="csv"){const rows=nonEmpty.flatMap(item=>item.rows.map(row=>({categoria:item.label,...row})));exportDownload(new Blob(["\ufeff"+exportCsv(rows)],{type:"text/csv;charset=utf-8"}),`finance-hub-dados-${stamp}.csv`)}
      else if(format==="separate"){nonEmpty.forEach((item,index)=>setTimeout(()=>exportDownload(new Blob(["\ufeff"+exportCsv(item.rows)],{type:"text/csv;charset=utf-8"}),`finance-hub-${item.table}-${stamp}.csv`),index*180))}
      else{const{jsPDF}=await import("jspdf"),pdf=new jsPDF(),period=start||end?`${start||"início"} a ${end||"hoje"}`:"Todo o período";pdf.setFontSize(18);pdf.text("Finance Hub · Exportação de dados",14,18);pdf.setFontSize(9);pdf.text(`Titular: ${owner.name||"Usuário"} · ${period} · Gerado em ${new Date().toLocaleString("pt-BR")}`,14,25);let y=35;nonEmpty.forEach(item=>{if(y>270){pdf.addPage();y=18}pdf.setFontSize(12);pdf.text(`${item.label} (${item.rows.length})`,14,y);y+=7;pdf.setFontSize(8);item.rows.forEach(row=>{if(y>282){pdf.addPage();y=18}const title=row.name||row.description||row.counterparty_name||row.participant_name||row.profile_name||row.nickname||"Registro",value=row.amount??row.total_amount??row.current_amount??row.remaining_amount,date=row.transaction_date||row.due_date||row.reference_month||row.created_at||"";pdf.text(`${String(date).slice(0,10)} · ${String(title).slice(0,62)}${value!==undefined?` · ${money(Number(value)||0)}`:""}`,16,y);y+=5});y+=4});pdf.save(`finance-hub-relatorio-${stamp}.pdf`)}
      notify(format==="pdf"?"Relatório PDF gerado.":format==="separate"?"Arquivos CSV separados gerados.":"Planilha CSV consolidada gerada.")
    }catch(error){notify(`Não foi possível exportar ${error.message||"os dados"}.`)}finally{setBusy("")}
  }
  return <div className="settings-panel data-export-panel"><div className="export-heading"><span><Database/></span><div><h2>Exportar meus dados</h2><p className="settings-help">Somente os dados da sua conta são incluídos. Os arquivos são gerados no seu dispositivo.</p></div></div><div className="export-filters"><label>Categoria<select value={group} onChange={e=>setGroup(e.target.value)}><option value="all">Todos os dados</option>{exportGroups.map(item=><option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label>Data inicial<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><label>Data final<input type="date" value={end} onChange={e=>setEnd(e.target.value)}/></label></div><div className="export-options"><button onClick={()=>run("csv")} disabled={!!busy}><Sheet/><span><strong>Planilha consolidada</strong><small>Um CSV compatível com Excel e Google Planilhas</small></span></button><button onClick={()=>run("pdf")} disabled={!!busy}><FileText/><span><strong>Relatório em PDF</strong><small>Resumo organizado para leitura e impressão</small></span></button><button onClick={()=>run("separate")} disabled={!!busy}><Download/><span><strong>CSV por tabela</strong><small>Arquivos separados com todos os campos originais</small></span></button></div>{busy&&<p className="export-progress"><RefreshCw/>Preparando sua exportação…</p>}<small className="export-privacy"><ShieldCheck/>A exportação não inclui senha nem credenciais de acesso.</small></div>
}
...
function SettingsModule({ owner, modules, reloadModules, onUpdate, dark, setDark, notify, ask, openBuilder }) {
  ...
  return (<React.Fragment>
    ...
    {section==="export"&&<DataExportPanel owner={owner} notify={notify}/>}
  </React.Fragment>);
}
...
createRoot(document.getElementById("root")).render(<ErrorBoundary><AuthGate renderApp={(owner) => <FinanceApp owner={owner} />} /></ErrorBoundary>);
